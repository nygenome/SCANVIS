...
#		    out$sj.samples=comma separated samples, RRSs and uniq.reads for each sj in out$sj
...
SCANVIS.merge<-function(scn,method='mean',roi=NULL,gen=NULL){
	...
	colnames(S0)=c('chr','start','end','JuncType','gene_name','FrameStatus','RRS','uniq.reads')
