...
Alternatively you can download the gz tar and install on R>=3.1.0 by executing the following command:
...
SCANVIS has four main functions: **SCANVIS.gencode, SCANVIS.scan, SCANVIS.linkvar, SCANVIS.merge, SCANVIS.visual**.
All SCANVIS functions require a gencode object generated by **SCANVIS.gencode**. The object supporting human gencode19 is supplied with the package while human gencode29 can be downloaded [here](gencode29.scanvis). Otherwise we recommend users first issue the following command using a suitable ftp url to a gencode directory of choice:

`gen=SCANVIS.gencode(<FTP.URL>)`

The next step is to score and annotate a set of splice junctions using the **SCANVIS.scan** function. A <SJ> 4 column matrix be prepared with columns labeled as "chr", "start", "end", "uniq.reads" to indicate the genomic coordinates and read support for the junctions. Note that such data is standard output from the [STAR](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4631051/) alignment software, but can be derived from any alignment software of choice. Users can then execute the following command:
...
Optionally, users can now map variants to the **SCANVIS.scan** output using the **SCANVIS.linkvar** function. A variant file <VARIANTS> must be prepared in bed format as a four column matrix with columns 1-3 being labeled "chr", "start", "end". The fourth column name should be a description of the variant type (eg. "ssMUT" would be a good title for splice site mutations) with entries describing the variant however users wish (eg. GT>C or rs123456). Variants can then be mapped to SJs by issuing the following command:
